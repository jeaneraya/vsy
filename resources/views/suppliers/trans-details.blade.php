...
<div class="container-fluid view-batch">
        ...
        <div class="row">
            <div class="col-6"><h2 class="main-title text-capitalize">{{$supplier_name}}'s Trans. Details</h2></div>
            <div class="col-6">
        ...
        <div class="collapse" id="additems">
          <div class="card card-body d-flex justify-content-center p-3">
            ...
            <form action="{{ route('add-supplier-items') }}" method="POST" id="form-product">
            @csrf
              <div class="row">
                <div class="col-3">
            ...
            <div class="trans-details table-wrapper products">
              <table id="trans-details-table" class="table table-striped posts-table align-middle" style="width:100%">
                ...
                @foreach($trans_details as $key => $td)
                    <tr>
                        <td>{{ $key + 1 }}</td>
                        <td class="t_product_code">{{ $td->item_code }}</td>
                        <td class="t_description">{{ $td->item_description }}</td>
      ...
      <script>
        $(document).ready(function() {
          $('#description').keyup(function() {
              ...
              if (query != '') {
                  $.ajax({
                      url: "{{ route('supplier-productcode-autocomplete') }}", 
                      method: "GET",
                      data: { query: query, supplier_id: {{ $supplier_id }} },
          ...
          $(document).on('click', '#supplier-products-list-ul li', function() {
...
<script>
  ...
  function updateProduct() {
      ...
      xhr.open('GET', '/get-supplier-product-price?product=' + encodeURIComponent(product) + '&unit=' + encodeURIComponent(unit));
      xhr.send();
  }
</script>
